...
const data: Category[] = [
  ...
  },
  {
    ...
    items:[
      ...
      {name:"그릭요거트 (그래놀라)",  nameEn:"Greek Yogurt (Granola)",            e:false, m:true,  s:false, p:false, w:false, pe:false, bs:false, c:false, to:false},
      {name:"그릭요거트 (딸기)",      nameEn:"Greek Yogurt (Strawberry)",         e:false, m:true,  s:false, p:false, w:false, pe:false, bs:false, c:false, to:false},
      {name:"그릭요거트 (블루베리)",  nameEn:"Greek Yogurt (Blueberry)",          e:false, m:true,  s:false, p:false, w:false, pe:false, bs:false, c:false, to:false},
      {name:"그릭요거트 (망고)",      nameEn:"Greek Yogurt (Mango)",              e:false, m:true,  s:false, p:false, w:false, pe:false, bs:false, c:false, to:false},
...
function Dot({ on }: { on: boolean }) {
  return on ? (
    <span style={{ display:"inline-block", width:11, height:11, borderRadius:"50%", background:"#174C35", flexShrink:0 }} />
  ) : null;
}

export default function AllergySheet({
  lang,
  onClose,
}: {
  lang: "ko" | "en";
  ...
  // Prevent scroll on body while sheet is open
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Sheet — centered, max-w-sm, from bottom */}
      <div
        className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm z-50 flex flex-col"
        style={{ maxHeight: "88vh" }}
      >
        <div className="bg-white rounded-t-3xl flex flex-col overflow-hidden" style={{ maxHeight: "88vh" }}>
          {/* Drag handle */}
          ...
          {/* Header */}
          <div className="px-5 pt-2 pb-3 flex items-center justify-between flex-shrink-0 border-b border-black/8">
            <h2 className="font-serif text-lg font-bold text-[#0D0D0D]">
              {lang === "ko" ? "알레르기 정보" : "Allergy Info"}
            </h2>
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full flex items-center justify-center text-[#888] hover:bg-black/5 transition-colors"
            >
          ...
          {/* Search + legend */}
          <div className="px-5 pt-3 pb-2 flex-shrink-0 flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-7 py-2 text-sm border border-black/15 outline-none focus:border-[#174C35] transition-all bg-white"
              />
              {query && (
                <button onClick={() => setQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#aaa] text-lg leading-none">×</button>
              )}
            </div>
            <span className="flex items-center gap-1.5 text-xs text-black/40 whitespace-nowrap flex-shrink-0">
              <span style={{ display:"inline-block", width:10, height:10, borderRadius:"50%", background:"#174C35" }} />
              {lang === "ko" ? "포함" : "Contains"}
            </span>
          </div>
          ...
          <div
            ref={scrollBoxRef}
            className="flex-1 overflow-auto mx-4 mb-4 border border-black/10"
          >
            <table style={{ width:"100%", borderCollapse:"collapse", minWidth:560 }}>
              <thead>
                <tr style={{ background:"#174C35" }}>
                  <th style={{ position:"sticky", top:0, left:0, zIndex:4, padding:"11px 14px", textAlign:"left", color:"#fff", fontWeight:700, fontSize:13, whiteSpace:"nowrap", borderRight:"1px solid rgba(255,255,255,0.15)", background:"#174C35" }}>
                    {lang === "ko" ? "메뉴" : "Menu"}
                  </th>
                  {COLS.map(col => (
                    <th key={col.key} style={{ position:"sticky", top:0, zIndex:2, padding:"11px 6px", textAlign:"center", color:"#fff", fontWeight:600, fontSize:11, whiteSpace:"nowrap", borderRight:"1px solid rgba(255,255,255,0.15)", minWidth:52, background:"#174C35" }}>
                  ...
                  return data.map((cat, ci) => (
                    <>
                      <tr key={`cat-${ci}`} style={{ background:"#E8F4EF" }}>
                        <td colSpan={10} style={{ position:"sticky", left:0, padding:"8px 14px", fontWeight:700, fontSize:12, color:"#174C35", borderTop:"1px solid #d0e8df", background:"#E8F4EF", zIndex:1 }}>
                      ...
                      {cat.items.map((item, ii) => {
                        ...
                        return (
                          <tr
                            ...
                            style={{ background:rowBg, borderTop:"1px solid #eef2f0", outline:matched ? "2px solid #D4A017" : "none", outlineOffset:matched ? "-2px" : undefined }}
                          >
                            <td style={{ position:"sticky", left:0, zIndex:1, padding:"10px 14px", fontSize:13, color:matched ? "#7A5800" : "#111", whiteSpace:"nowrap", borderRight:"1px solid #eef2f0", fontWeight:matched ? 700 : 500, background:rowBg }}>
                              {lang === "ko" ? item.name : item.nameEn}
                            </td>
                            {COLS.map(col => (
                              <td key={col.key} style={{ padding:"10px 6px", textAlign:"center", borderRight:"1px solid #eef2f0" }}>
